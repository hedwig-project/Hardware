...
void display_config(String msg)
{
    ...
    messageToSend += display_confirm(now(),displaytype,backlight);
}
...
{
    if (timeStatus() != timeNotSet) messageToSend += UmiTempPresReles(now(),80,25,1,1,0);
    if (timeStatus() != timeNotSet) messageToSend += UmiTempPresReles(now(),50,40,1,1,0);
